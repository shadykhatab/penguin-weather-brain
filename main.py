...
# --- DIRECT API CALL (The Final Version) ---
def talk_to_google_direct(prompt, weather_context):
    if not API_KEY: return "I lost my API key!"
    
    # WE USE THE WORKING MODEL FROM YOUR SCREENSHOT: gemini-2.5-flash
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={API_KEY}"
    
    headers = {'Content-Type': 'application/json'}
    
    # We hide the instructions inside the 'system' part of the prompt
    full_prompt = f"""
    System: You are a funny, sarcastic AI Penguin assistant.
    Context: The user is in a location where the weather is: {weather_context}.
...
def chat():
    ...
    # 3. ASK GOOGLE
    reply = talk_to_google_direct(user_msg, weather_info)

    return jsonify({"reply": reply})

@app.route('/weather', methods=['GET'])
def weather():
    # Keep the Home Screen working
    city = request.args.get('city', 'New York')
    # For now, return static data to be safe, or you can use get_live_weather here too
